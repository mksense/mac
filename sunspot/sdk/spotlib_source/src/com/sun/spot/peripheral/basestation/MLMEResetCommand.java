...
public class MLMEResetCommand extends MACCommand {
	private boolean resetAttribs;
	
	public ICommand with(boolean resetAttribs) {
		this.resetAttribs = resetAttribs;
		return this;
	}
	
	protected void writeParametersOnto(DataOutputStream dataOutputStream) throws IOException {
		dataOutputStream.writeBoolean(resetAttribs);
	}
	
	public void populateFrom(byte[] inputBuffer, int startingOffset) throws IOException {
		resetAttribs = inputBuffer[startingOffset] == 1;
	}
	
	protected void prepareResultOrExecute(I802_15_4_MAC mac) {
		mac.mlmeReset(resetAttribs);
	}
	
	protected byte classIndicatorByte() {
		return MLMEResetCommand;
	}
}
