...
public class ClaimCommand extends PIOCommand {

	int mask;
	int drive;
	boolean claimForPeriphA;
	
	public ClaimCommand(int pioSelector, int mask, int drive, boolean claimForPeriphA) {
		super(pioSelector);
		this.mask = mask;
		this.drive = drive;
		this.claimForPeriphA = claimForPeriphA;
	}

	public ReplyEnvelope execute(IAT91_PIO pio) throws Exception {
		pio.claim(mask, drive, claimForPeriphA);
