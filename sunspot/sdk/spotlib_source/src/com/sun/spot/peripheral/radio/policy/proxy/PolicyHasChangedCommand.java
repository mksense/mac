...
class PolicyHasChangedCommand extends RequestEnvelope {
	private IConnectionID cid;
	private RadioPolicy selection;
	
	public PolicyHasChangedCommand(IConnectionID cid, RadioPolicy selection) {
		this.cid = cid;
		this.selection = selection;
	}

	public ReplyEnvelope execute(Object context) {
		RadioPolicy localPolicy = RadioPolicy.policyMatching(selection);
		Spot.getInstance().getRadioPolicyManager().policyHasChanged(cid, localPolicy);
